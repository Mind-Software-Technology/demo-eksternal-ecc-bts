...
export const api = {
  ...
  orders: {
    // Creates the order from the current cart (guest_name/guest_phone only —
    // no attachments here anymore). Attachments are uploaded per item in a
    // separate step, after the WhatsApp consultation — see uploadAttachment.
    ...
    // Uploads/replaces the attachment for one order item. Allowed while the
    // order is awaiting_quote or quoted (i.e. before the customer accepts
    // and moves to payment).
    // Adds one attachment for the item — doesn't replace previous ones. Items
    // ordered with qty > 1 can carry up to `qty` attachments this way.
    uploadAttachment: (orderNo, itemId, file) =>
      requestMultipart(`/api/orders/${orderNo}/items/${itemId}/attachment`, 'POST', { attachment: file }),
    deleteAttachment: (orderNo, itemId, attachmentId) =>
      request(`/api/orders/${orderNo}/items/${itemId}/attachment/${attachmentId}`, { method: 'DELETE' }),
    // Plain links (not fetch calls) — the browser's top-level GET navigation
    // still carries the Sanctum session cookie (SameSite=Lax allows that),
    // so an <a href> straight to these URLs is enough to trigger the download.
    // One item can carry several attachments/results (up to its qty) — these
    // download one specific file by id, matching the `attachments`/`results`
    // arrays OrderItemResource returns.
    attachmentFileUrl: (orderNo, itemId, attachmentId) =>
      `${BASE_URL}/api/orders/${orderNo}/items/${itemId}/attachment/${attachmentId}`,
    resultFileUrl: (orderNo, itemId, resultId) =>
      `${BASE_URL}/api/orders/${orderNo}/items/${itemId}/result/${resultId}`,
    submitTestimonial: (orderNo, payload) =>
      request(`/api/orders/${orderNo}/testimonial`, { method: 'POST', body: payload }),
    acceptQuote: (orderNo) => request(`/api/orders/${orderNo}/accept-quote`, { method: 'POST' }),
    declineQuote: (orderNo) => request(`/api/orders/${orderNo}/decline`, { method: 'POST' }),
  },
  // Lonceng notifikasi — sumbernya tabel `notifications` Laravel, sama dengan
  // yang dikirim lewat web push, jadi keduanya tidak pernah berbeda isi.
  notifications: {
    list: () => request('/api/notifications'),
    markAllRead: () => request('/api/notifications/read', { method: 'POST' }),
  },
  push: {
    // Terima langsung hasil PushSubscription.toJSON() dari browser.
    subscribe: (subscription) =>
      request('/api/push-subscriptions', { method: 'POST', body: subscription }),
    unsubscribe: (endpoint) =>
      request('/api/push-subscriptions', { method: 'DELETE', body: { endpoint } }),
  },
  payments: {
    ...
    // Cancels the pending transaction but keeps the order payable (status
    // goes back to 'pending' instead of 'cancelled') — for when the customer
    // picked the wrong method and wants to try a different one.
    changeMethod: (orderNo) => request(`/api/payments/${orderNo}/change-method`, { method: 'POST' }),
    uploadProof: (orderNo, file) =>
      requestMultipart(`/api/payments/${orderNo}/proof`, 'POST', { proof: file }),
  },
  auth: {
